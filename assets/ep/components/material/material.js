	...
	var material = {
		...
		'playaudio' : function(){
			$("#voice, .panel").on('click', '.audio-player-play', function(){
				...
				player.jPlayer({
					...
					swfPath: "assets/ep/components/jplayer",
		...
		'buildHtml' : function() {
			...
			dialog['mainDialog'] = '<div id="material-Modal" class="uploader-modal modal fade" tabindex="-1" role="dialog" aria-hidden="true">\n' +
				...
				'				<div id="btn-select">\n' +
				'					<span class="btn btn-primary" data-dismiss="modal">取消</span>\n' +
			...
			dialog['musicDialog'] = '<div class="form-group">'+
				...
				'			<button class="btn btn-primary audio-player-play" type="button" style="display:none;"><i class="fa fa-play"></i></button>'+
				'			<button class="btn btn-primary" type="button" onclick="showAudioDialog(this);">选择媒体文件</button>'+
				'		</span>'+
				'	</div>'+
			...
			dialog['voiceDialog'] ='<table class="table table-hover we7-table" style="margin-bottom:0">'+
				...
				'								<td align="center">'+
				'									<div class="btn-group">'+
				'										<a href="javascript:;" class="btn btn-primary btn-sm audio-player-play audio-msg" data-attach="<%=item.attach%>"><i class="fa fa-play"></i></a>'+
				'										<a href="javascript:;" class="btn btn-primary btn-sm checkMedia" data-media="<%=item.media_id%>" data-type="voice" data-attachid="<%=item.id%>">选取</a>'+
				'									</div>'+
				'								</td>'+
				'							</tr>'+
				'							<%});%>' +
			...
			dialog['videoDialog'] ='<table class="table table-hover we7-table" style="margin-bottom:0">'+
				...
				'								<td align="center">'+
				'									<div class="btn-group">'+
				'										<a href="javascript:;" class="btn btn-primary btn-sm checkMedia" data-media="<%=item.media_id%>" data-type="video" data-attachid="<%=item.id%>">选取</a>'+
				'									</div>'+
				'								</td>'+
				'							</tr>'+
				'							<%});%>' +
			...
			dialog['wxcardDialog'] ='<table class="table table-hover we7-table">\n'+
				...
				'						<td><a href="javascript:;" class="btn btn-primary btn-sm checkMedia" data-title="<%=item.title%>" data-type="wxcard" data-media="<%=item.card_id%>" data-attachid="<%=item.id%>">选取</a></td>' +
				'					</tr>' +
				'					<%});%>' +
				'				</tbody>'+
				'   		</table>';
