...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim">'
    ...
    ,'<div class="layim-tab-content">'
      ,'<ul class="layim-list-top">'
        ...
        ,'<li layim-event="newFriend"><i class="mdi mdi-delete-forever-outline">&#xe654;</i>新的朋友<i class="layim-new" id="LAY_layimNewFriend"></i></li>'
        ,'{{# } if(d.base.isgroup){ }}'
        ,'<li layim-event="group"><i class="mdi mdi-delete-forever-outline">&#xe613;</i>群聊<i class="layim-new" id="LAY_layimNewGroup"></i></li>'
        ,'{{# } }}'
      ,'</ul>'
      ,'<ul class="layim-list-friend">'
        ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
        ,'<li>'
          ,'<h5 layim-event="spread" lay-type="{{ spread }}"><i class="mdi mdi-delete-forever-outline">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
    ...
    ,'<div class="layim-tab-content">'
      ,'<ul class="layim-list-top">'
        ...
        ,'</li>'
        ,'{{# }); if(!d.base.copyright){ }}'
        ,'<li layim-event="about"><i class="mdi mdi-delete-forever-outline">&#xe60b;</i>关于<i class="layim-new" id="LAY_layimNewAbout"></i></li>'
        ,'{{# } }}'
      ,'</ul>'
    ,'</div>'
  ,'</div>'
  ,'<ul class="layui-unselect layui-layim-tab">'
    ,'<li title="消息" layim-event="tab" lay-type="message" class="layim-this"><i class="mdi mdi-delete-forever-outline">&#xe611;</i><span>消息</span><i class="layim-new" id="LAY_layimNewMsg"></i></li>'
    ,'<li title="联系人" layim-event="tab" lay-type="friend"><i class="mdi mdi-delete-forever-outline">&#xe612;</i><span>联系人</span><i class="layim-new" id="LAY_layimNewList"></i></li>'
    ,'<li title="更多" layim-event="tab" lay-type="more"><i class="mdi mdi-delete-forever-outline">&#xe670;</i><span>更多</span><i class="layim-new" id="LAY_layimNewMore"></i></li>'
  ,'</ul>'].join('');
  ...
  layui.data.content = function(content){
    ...
    .replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g, function(str){ //转义文件
      ...
      if(!href) return str;
      return '<a class="layui-layim-file" href="'+ href +'" download target="_blank"><i class="mdi mdi-delete-forever-outline">&#xe61e;</i><cite>'+ (text||href) +'</cite></a>';
    })
    .replace(/audio\[([^\s]+?)\]/g, function(audio){  //转义音频
      return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" data-src="' + audio.replace(/(^audio\[)|(\]$)/g, '') + '"><i class="mdi mdi-delete-forever-outline">&#xe652;</i><p>音频消息</p></div>';
    })
    .replace(/video\[([^\s]+?)\]/g, function(video){  //转义音频
      return '<div class="layui-unselect layui-layim-video" layim-event="playVideo" data-src="' + video.replace(/(^video\[)|(\]$)/g, '') + '"><i class="mdi mdi-delete-forever-outline">&#xe652;</i></div>';
    })
