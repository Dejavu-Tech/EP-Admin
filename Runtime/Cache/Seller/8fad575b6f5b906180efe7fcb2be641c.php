				...
				<div class="page-header">
					<div class="row">
						<div class="col-lg-6 main-header">
							...
							<h2>商品标签<span>管理</span></h2>
						</div>
						<div class="col-lg-6 breadcrumb-right" style="float: right">
							<ol class="breadcrumb">
								<a href="<?php echo U('goods/addtags', array('ok' => 1));?>"><button class="btn btn-info btn-air-info" style=""><i class="fa fa-plus"></i> 添加商品标签</button></a>
							</ol>
						</div>
					</div>
				</div>
								...
								<form action="" class="form theme-form layui-form m-t-20" lay-filter="example" method="post" >
									<div class="dataTables_wrapper no-footer">
										<div class="page-table-header m-b-20 row">
											...
											<div class="btn-group">
												<button class="btn btn-pill btn-primary btn-sm  btn-operation"  type="button" data-toggle='batch' data-href="<?php echo U('goods/tagsstate',array('state'=>1));?>">
													启用
												</button>
												<button class="btn btn-pill btn-primary btn-sm  btn-operation"  type="button" data-toggle='batch'  data-href="<?php echo U('goods/tagsstate',array('state'=>0));?>">
													禁用
												</button>
												<button class="btn btn-pill btn-primary btn-sm  btn-operation"  type="button" data-toggle='batch-remove' data-confirm="确认要删除吗?" data-href="<?php echo U('goods/deletetags');?>">
													删除
												</button>
											</div>
										</div>
										<div class="table-responsive">
											<table class="display dataTable text-center" lay-even lay-skin="line" lay-size="lg">
												...
												<tr>
													...
													<?php }else{ ?>
													<td><img style="width: 60px;" src="<?php echo tomedia($row['tagcontent']);?>"></td>
													<?php } ?>
													<td>
														<label class="switch" style="margin-bottom: 0">
															<input type="checkbox" name=""   lay-filter="statewsitch" data-href="<?php echo U('goods/tagsstate',array('id'=>$row['id']));?>" <?php if($row['state']==1){ ?>checked<?php }else{ } ?> lay-skin="switch">
														</label>
													</td>

													<td class="text-center">
														<a class="btn btn-pill btn-primary btn-sm" href="<?php echo U('goods/edittags', array('id' => $row['id'], 'ok' => 1));?>"  >
															编辑
														</a>
														<?php if($row['id']!='default'){ ?>
														<a class="btn btn-pill btn-primary btn-sm deldom" href="javascript:;" data-href="<?php echo U('goods/deletetags', array('id' => $row['id']));?>" data-confirm='确认要删除此标签组吗?'>
															删除
														</a>
														<?php } ?>
													</td>
