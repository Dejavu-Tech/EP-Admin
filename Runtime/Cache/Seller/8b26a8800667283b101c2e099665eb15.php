				...
				<div class="page-header">
					<div class="row">
						<div class="col-lg-6 main-header">
							...
							<h2>商品评价<span>列表</span></h2>
						</div>
						<div class="col-lg-6 breadcrumb-right" style="float: right">
							<ol class="breadcrumb">
								<a  href="<?php echo U('goods/addvircomment', array('ok' => 1));?>"><button class="btn btn-info btn-air-info" style=""><i class="fa fa-plus"></i>添加虚拟评价</button></a>
							</ol>
						</div>
					</div>
				</div>
							...
							<div class="card-body" >
								<form action="" method="get" class="form-horizontal form-search layui-form">
									...
									<div class="form-row m-b-20">
										<div class="p-r-5">
											<input type="text" class="form-control" name='keyword' value="<?php echo ($_GPC['keyword']); ?>" placeholder="输入关键词点击搜索">
										</div>
										<div class="p-r-5">
								...
								<form action="" class="form theme-form layui-form m-t-20" lay-filter="example" method="post" >
									<div class="dataTables_wrapper no-footer">
										<div class="page-table-header m-b-20 row">
											...
											<div class="btn-group">
												<button class="btn btn-pill btn-primary btn-sm btn-operation" type="button" data-toggle='batch' data-href="<?php echo U('goods/commentstate',array('state'=>1));?>">
													启用
												</button>
												<button class="btn btn-pill btn-primary btn-sm btn-operation" type="button" data-toggle='batch'  data-href="<?php echo U('goods/commentstate',array('state'=>0));?>">
													禁用
												</button>
												<button class="btn btn-pill btn-primary btn-sm btn-operation" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php echo U('goods/deletecomment');?>">
													删除
												</button>
											</div>
										</div>
												...
												<tr>
													<td>
														<div class="checkbox checkbox-primary m-l-10" >
															<input type='checkbox' name="item_checkbox" lay-skin="primary" value="<?php echo ($row['comment_id']); ?>" style="display: none"/>
														</div>
													</td>
													<td>
														<img class="img-40" src="<?php echo tomedia($row['goods_image']);;?>" style="border-radius:5px;">
														<div><?php echo ($row['goods_name']); ?></div>
														<div><?php echo ($row['order_num_alias']); ?></div>
													</td>
													<td>
														<img class="img-40" src="<?php echo tomedia($row['avatar']);;?>" style="border-radius:5px;">
													...
													<td>
														<?php
 $images_str = unserialize($row['images']); $images_arr = explode(',', $images_str); ?>
														<?php foreach( $images_arr as $img ){ ?>
														<?php  if($img != 'undefined' && !empty($img) ){ ?>
														<img class="img-40" src="<?php echo tomedia($img);;?>" style="border-radius:5px;">
														<?php } ?>
														<?php } ?>
													</td>
													<td>
														描述星级:<?php echo ($row['star']); ?><br/>
														价格合理星级:<?php echo ($row['star2']); ?><br/>
														质量满意星级:<?php echo ($row['star3']); ?><br/>
													</td>
													<td>
													...
													<td>
														<label class="switch" style="margin-bottom: 0">
															<input type="checkbox" name="" lay-filter="statewsitch" data-href="<?php echo U('goods/commentstate',array('id'=>$row['comment_id']));?>" <?php if($row['state']==1){ ?>checked<?php }else{ } ?> lay-skin="switch" >
														</label>
													</td>
													<td>
														<a href="javascript:;" data-href="<?php echo U('goods/deletecomment', array('id' => $row['comment_id']));?>" data-confirm='确认要删除吗?' class="btn btn-pill btn-primary btn-sm deldom">删除</a>
													</td>
												</tr>
												<?php } ?>
												</tbody>
